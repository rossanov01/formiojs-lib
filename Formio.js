  ...
  function Formio(path) {
    ...
    if (!this.noProject) {
      ...
      if (isProjectUrl) {
        ...
        // Модификация: добавляем поддержку для всех окружений включая QA серверы
        if (hostparts.length > 2 && (
          hostparts[2].split('.').length > 2 || 
          hostName.includes('localhost') ||
          hostName.includes('unim.internal') ||
          hostName.includes('dpathology') ||
